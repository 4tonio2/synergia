...
/**
 * Fetch calendar events for a user from Odoo
 */
...
/**
 * Generate embedding for a name using OpenAI
 */
...
/**
 * Match contact by vector similarity using Supabase RPC
 */
...
/**
 * Match multiple contacts in parallel
 */
...
/**
 * Extract event info (names, location, reason, dates) from transcript using OpenAI
 */
...
async function handleWelcome(req: VercelRequest, res: VercelResponse) {
	...
	// Fetch calendar events if caller identified
	let events: CalendarEvent[] = [];
	if (caller?.id) {
		events = await fetchCallerEvents(caller.id);
	}

	// Build caller params for next hooks
...
function handleMainMenu(req: VercelRequest, res: VercelResponse) {
	const payload = req.body as JambonzWebhookPayload;
	const baseUrl = getBaseUrl();
...
async function handleSelectEventToCancel(req: VercelRequest, res: VercelResponse) {
	const payload = req.body as JambonzWebhookPayload;
	...
	try {
		...
		const { events, callerId, callerName } = data;
...
async function handleConfirmDelete(req: VercelRequest, res: VercelResponse) {
	const payload = req.body as JambonzWebhookPayload;
	const dataParam = req.query.data as string;
...
async function handleProcessBooking(req: VercelRequest, res: VercelResponse) {
	...
	if (payload.reason === 'speechDetected' && payload.speech?.alternatives?.length) {
		...
		try {
			// 1. Extract event info (names, location, reason, dates) using OpenAI
			...
			// 2. Match contacts in parallel using embeddings
			...
			// 3. Handle unmatched participants first
			...
			// 4. Handle ambiguous participants
			...
			// 5. All matched - proceed to confirmation
...
async function proceedToConfirmation(req: VercelRequest, res: VercelResponse, agendaResult: any, callerId: string, callerName: string) {
	const baseUrl = getBaseUrl();
...
async function handleFinalConfirmation(req: VercelRequest, res: VercelResponse) {
	const payload = req.body as JambonzWebhookPayload;
	const dataParam = req.query.data as string;
	const baseUrl = getBaseUrl();
	...
	try {
		...
		if (checkResponse.ok) {
			...
			// If times changed, it means conflict was found
...
async function handleCreateMissingContact(req: VercelRequest, res: VercelResponse) {
	...
	try {
		...
		if (createResponse.ok) {
			...
			// Update agendaResult with new match
			...
			// Check for remaining unmatched/ambiguous
			...
			// All done - proceed to confirmation
...
async function handleConfirmParticipant(req: VercelRequest, res: VercelResponse) {
	...
	try {
		...
		if (choice === 0) {
			// None of the candidates - ask for phone number
			const contextData = encodeURIComponent(JSON.stringify({
				targetName,
				agendaResult,
				callerId
		...
		// Update agendaResult with selected match
		const updatedParticipants = agendaResult.participants.map((p: ParticipantMatch) => {
			if (p.input_name === targetName) {
				return {
					...p,
		...
		// Check for remaining ambiguous/unmatched
		const ambiguous = updatedParticipants.filter((p: ParticipantMatch) => p.status === 'ambiguous');
		const unmatched = updatedParticipants.filter((p: ParticipantMatch) => p.status === 'unmatched');
